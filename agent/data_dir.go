...
func getDataDir(dataDirs ...string) (string, error) {
	...
	if runtime.GOOS == "windows" {
		dataDirs = append(dataDirs,
			filepath.Join(os.Getenv("APPDATA"), "aether-agent"),
			filepath.Join(os.Getenv("LOCALAPPDATA"), "aether-agent"),
		)
	} else {
		dataDirs = append(dataDirs, "/var/lib/aether-agent")
		if homeDir, err := os.UserHomeDir(); err == nil {
			dataDirs = append(dataDirs, filepath.Join(homeDir, ".config", "aether"))
		}
	}
	return testDataDirs(dataDirs)
}
