...
func TestNewWebSocketClient(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			// Set up environment
			if tc.hubURL != "" {
				os.Setenv("AETHER_AGENT_HUB_URL", tc.hubURL)
			} else {
				os.Unsetenv("AETHER_AGENT_HUB_URL")
			}
			if tc.token != "" {
				os.Setenv("AETHER_AGENT_TOKEN", tc.token)
			} else {
				os.Unsetenv("AETHER_AGENT_TOKEN")
			}
			defer func() {
				os.Unsetenv("AETHER_AGENT_HUB_URL")
				os.Unsetenv("AETHER_AGENT_TOKEN")
...
func TestWebSocketClient_GetOptions(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			expectedScheme: "ws",
			expectedPath:   "/api/aether/agent-connect",
		},
		{
		...
		},
		{
			...
			expectedScheme: "ws",
			expectedPath:   "/custom/path/api/aether/agent-connect",
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			// Set up environment
			os.Setenv("AETHER_AGENT_HUB_URL", tc.inputURL)
			os.Setenv("AETHER_AGENT_TOKEN", "test-token")
			defer func() {
				os.Unsetenv("AETHER_AGENT_HUB_URL")
				os.Unsetenv("AETHER_AGENT_TOKEN")
			}()
			...
			assert.Equal(t, aether.Version, options.RequestHeader.Get("X-Aether"))
...
func TestWebSocketClient_VerifySignature(t *testing.T) {
	...
	// Set up environment
	os.Setenv("AETHER_AGENT_HUB_URL", "http://localhost:8080")
	os.Setenv("AETHER_AGENT_TOKEN", "test-token")
	defer func() {
		os.Unsetenv("AETHER_AGENT_HUB_URL")
		os.Unsetenv("AETHER_AGENT_TOKEN")
...
func TestGetToken(t *testing.T) {
	unsetEnvVars := func() {
		os.Unsetenv("AETHER_AGENT_TOKEN")
		os.Unsetenv("TOKEN")
		os.Unsetenv("AETHER_AGENT_TOKEN_FILE")
	...
	t.Run("token from AETHER_AGENT_TOKEN environment variable", func(t *testing.T) {
		unsetEnvVars()

		// Set AETHER_AGENT_TOKEN env var (should take precedence)
		expectedToken := "test-token-from-aether-env"
		os.Setenv("AETHER_AGENT_TOKEN", expectedToken)
		defer os.Unsetenv("AETHER_AGENT_TOKEN")
	...
	t.Run("token from AETHER_AGENT_TOKEN_FILE", func(t *testing.T) {
		unsetEnvVars()

		// Create a temporary token file
		expectedToken := "test-token-from-aether-file"
		tokenFile, err := os.CreateTemp("", "token-test-*.txt")
		require.NoError(t, err)
		defer os.Remove(tokenFile.Name())
		...
		// Set AETHER_AGENT_TOKEN_FILE env var (should take precedence)
		os.Setenv("AETHER_AGENT_TOKEN_FILE", tokenFile.Name())
		defer os.Unsetenv("AETHER_AGENT_TOKEN_FILE")
