...
//go:generate dotnet build -c Release lhm/aether_lhm.csproj
...
var (
	aetherLhm     *lhmProcess
	aetherLhmOnce sync.Once
...
func newlhmProcess() (*lhmProcess, error) {
	destDir := filepath.Join(os.TempDir(), "aether")
	execPath := filepath.Join(destDir, "aether_lhm.exe")
...
func getSensorTemps(ctx context.Context) (temps []sensors.TemperatureStat, err error) {
	...
	aetherLhmOnce.Do(func() {
		aetherLhm, err = newlhmProcess()
	})
