// Package agent implements the Aether monitoring agent that collects and serves system metrics.
//
// The agent runs on monitored systems and communicates collected data
// to the Aether hub for centralized monitoring and alerting.
package agent
...
// GetEnv retrieves an environment variable with an "AETHER_AGENT_" prefix, or falls back to the unprefixed key.
func GetEnv(key string) (value string, exists bool) {
	if value, exists = os.LookupEnv("AETHER_AGENT_" + key); exists {
		return value, exists
	}
