...
func TestStartServer(t *testing.T) {
	...
	socketFile := filepath.Join(t.TempDir(), "aether-test.sock")
...
func TestExtractHubVersion(t *testing.T) {
	tests := []struct {
		...
		{
			name:            "valid aether client version with underscore",
			clientVersion:   "SSH-2.0-aether_0.11.1",
			expectedVersion: "0.11.1",
			expectError:     false,
		},
		{
			name:            "valid aether client version with beta",
			clientVersion:   "SSH-2.0-aether_1.0.0-beta",
			expectedVersion: "1.0.0-beta",
			expectError:     false,
		},
		{
			name:            "valid aether client version with rc",
			clientVersion:   "SSH-2.0-aether_0.12.0-rc1",
		...
		},
		{
			...
			clientVersion:   "aether_",
			expectedVersion: "",
			expectError:     true,
		},
		{
			name:            "version with patch and build metadata",
			clientVersion:   "SSH-2.0-aether_1.2.3+build.123",
...
func TestGetHubVersion(t *testing.T) {
	...
	mockCtx := &mockSSHContext{
		...
		clientVersion: "SSH-2.0-aether_0.12.0",
	}
	...
	mockCtx.clientVersion = "SSH-2.0-aether_0.11.0" // Change version but should still return cached
	version = agent.getHubVersion("test-session-123", mockCtx)
	...
	// Test with invalid version string (non-aether client)
	mockCtx.clientVersion = "SSH-2.0-OpenSSH_8.0"
	version = agent.getHubVersion("invalid-session", mockCtx)
	assert.Equal(t, "0.0.0", version.String()) // Should be empty version for non-aether clients
...
func TestHubVersionCaching(t *testing.T) {
	...
	ctx2 := &mockSSHContext{
		sessionID:     "session2",
		clientVersion: "SSH-2.0-aether_0.11.0",
	}
	...
	ctx1.clientVersion = "SSH-2.0-aether_0.10.0"
	...
	ctx3 := &mockSSHContext{
		sessionID:     "session3",
		clientVersion: "SSH-2.0-aether_0.13.0",
	}
