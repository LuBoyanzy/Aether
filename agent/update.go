...
// restarter knows how to restart the aether-agent service.
...
func (s *systemdRestarter) Restart() error {
	...
	if err := exec.Command(s.cmd, "is-active", "aether-agent.service").Run(); err != nil {
		return nil
	}
	ghupdate.ColorPrint(ghupdate.ColorYellow, "Restarting aether-agent.service via systemd…")
	return exec.Command(s.cmd, "restart", "aether-agent.service").Run()
}

type openRCRestarter struct{ cmd string }

func (o *openRCRestarter) Restart() error {
	if err := exec.Command(o.cmd, "aether-agent", "status").Run(); err != nil {
		return nil
	}
	ghupdate.ColorPrint(ghupdate.ColorYellow, "Restarting aether-agent via OpenRC…")
	return exec.Command(o.cmd, "aether-agent", "restart").Run()
}
...
func (w *openWRTRestarter) Restart() error {
	...
	if err := exec.Command("/etc/init.d/aether-agent", "running").Run(); err != nil {
		return nil
	}
	ghupdate.ColorPrint(ghupdate.ColorYellow, "Restarting aether-agent via procd…")
	return exec.Command("/etc/init.d/aether-agent", "restart").Run()
}

type freeBSDRestarter struct{ cmd string }

func (f *freeBSDRestarter) Restart() error {
	if err := exec.Command(f.cmd, "aether-agent", "status").Run(); err != nil {
		return nil
	}
	ghupdate.ColorPrint(ghupdate.ColorYellow, "Restarting aether-agent via FreeBSD rc…")
	return exec.Command(f.cmd, "aether-agent", "restart").Run()
}
...
// Update checks GitHub for a newer release of aether-agent, applies it,
...
func Update(useMirror bool) error {
	...
	// set ownership to aether:aether if possible
	if chownPath, err := exec.LookPath("chown"); err == nil {
		if err := exec.Command(chownPath, "aether:aether", exePath).Run(); err != nil {
