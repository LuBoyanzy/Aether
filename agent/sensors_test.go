...
func TestNewSensorConfig(t *testing.T) {
	...
	originalPrimary, hasPrimary := os.LookupEnv("AETHER_AGENT_PRIMARY_SENSOR")
	originalSys, hasSys := os.LookupEnv("AETHER_AGENT_SYS_SENSORS")
	originalSensors, hasSensors := os.LookupEnv("AETHER_AGENT_SENSORS")

	// Restore environment variables after the test
	defer func() {
		// Clean up test environment variables
		os.Unsetenv("AETHER_AGENT_PRIMARY_SENSOR")
		os.Unsetenv("AETHER_AGENT_SYS_SENSORS")
		os.Unsetenv("AETHER_AGENT_SENSORS")

		// Restore original values if they existed
		if hasPrimary {
			os.Setenv("AETHER_AGENT_PRIMARY_SENSOR", originalPrimary)
		}
		if hasSys {
			os.Setenv("AETHER_AGENT_SYS_SENSORS", originalSys)
		}
		if hasSensors {
			os.Setenv("AETHER_AGENT_SENSORS", originalSensors)
		}
	}()

	// Set test environment variables
	os.Setenv("AETHER_AGENT_PRIMARY_SENSOR", "test_primary")
	os.Setenv("AETHER_AGENT_SYS_SENSORS", "/test/path")
	os.Setenv("AETHER_AGENT_SENSORS", "test_sensor1,test_*,test_sensor3")
