...
# Function to find agent binary (prefers aether name, falls back to legacy package locations)
function Find-AetherAgent {
    $agentNames = @($ServiceName, $LegacyPackageName)
    foreach ($name in $agentNames) {
        $agentCmd = Get-Command $name -ErrorAction SilentlyContinue
        if ($agentCmd) {
    ...
    $commonPaths = @(
        "$env:USERPROFILE\scoop\apps\$LegacyPackageName\current\$LegacyPackageName.exe",
        "$env:USERPROFILE\scoop\apps\$ServiceName\current\$ServiceName.exe",
        "$env:ProgramData\scoop\apps\$LegacyPackageName\current\$LegacyPackageName.exe",
        "$env:ProgramData\scoop\apps\$ServiceName\current\$ServiceName.exe",
        "$env:LOCALAPPDATA\Microsoft\WinGet\Packages\$LegacyWingetId*\$LegacyPackageName.exe",
        "$env:ProgramFiles\WinGet\Packages\$LegacyWingetId*\$LegacyPackageName.exe",
        "${env:ProgramFiles(x86)}\WinGet\Packages\$LegacyWingetId*\$LegacyPackageName.exe",
        "$env:ProgramFiles\$ServiceName\$ServiceName.exe",
        "$env:ProgramFiles(x86)\$ServiceName\$ServiceName.exe",
        "$env:SystemDrive\Users\*\scoop\apps\$LegacyPackageName\current\$LegacyPackageName.exe"
    )
...
# Function to upgrade agent with Scoop (legacy package)
function Upgrade-AetherAgentWithScoop {
    Write-Host "Upgrading $LegacyPackageName with Scoop..."
    scoop update $LegacyPackageName
    
    if (-not (Test-CommandExists $LegacyPackageName)) {
        throw "Failed to upgrade $LegacyPackageName with Scoop"
    }
    
    return $(Join-Path -Path $(scoop prefix $LegacyPackageName) -ChildPath "$LegacyPackageName.exe")
}

# Function to upgrade agent with WinGet (legacy package id)
function Upgrade-AetherAgentWithWinGet {
    Write-Host "Upgrading $LegacyPackageName with WinGet..."
    ...
    & winget upgrade --exact --id $LegacyWingetId --accept-source-agreements --accept-package-agreements | Out-Null
    ...
    # Refresh PATH environment variable to make the agent available in current session
    $env:Path = [System.Environment]::GetEnvironmentVariable("Path", "Machine") + ";" + [System.Environment]::GetEnvironmentVariable("Path", "User")
    
    # Find the path to the agent executable
    $agentPath = Find-AetherAgent
    
    if (-not $agentPath) {
        throw "Could not find agent executable path after upgrade"
    }
...
function Get-ServiceConfiguration {
    ...
    if (-not $existingService) {
        throw "$ServiceName service does not exist. Please run the installation script first."
    }
    ...
    try {
        ...
        $envVars = & $nssmCommand get $ServiceName AppEnvironmentExtra
...
function Update-ServicePath {
    ...
    Write-Host "Updating $ServiceName service path..."
    ...
    & $nssmCommand set $ServiceName Application $NewAgentPath
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to update $ServiceName service path"
    }
    
    Write-Host "Service path updated to: $NewAgentPath"
    
    # Start the service
    Start-AetherAgentService -NSSMPath $nssmCommand
}

# Function to start and monitor the service
function Start-AetherAgentService {
    param (
        [string]$NSSMPath = ""
    )
...
try {
    Write-Host "$ProgramDisplayName Upgrade Script" -ForegroundColor Cyan
    Write-Host "===============================" -ForegroundColor Cyan
    
    # First: Check if service exists (doesn't require admin)
    $existingService = Get-Service -Name $ServiceName -ErrorAction SilentlyContinue
    if (-not $existingService) {
        Write-Host "ERROR: $ServiceName service does not exist." -ForegroundColor Red
    ...
    Write-Host "Upgrading $ProgramDisplayName (via legacy package)..."
    $newAgentPath = $null
    
    if (Test-CommandExists "scoop") {
        Write-Host "Using Scoop for upgrade..."
        $newAgentPath = Upgrade-AetherAgentWithScoop
    }
    elseif (Test-CommandExists "winget") {
        Write-Host "Using WinGet for upgrade..."
        $newAgentPath = Upgrade-AetherAgentWithWinGet
    }
    else {
    ...
    if (-not $newAgentPath) {
        $newAgentPath = Find-AetherAgent
        if (-not $newAgentPath) {
            throw "Could not find agent executable after upgrade."
        }
    }
    ...
    if ($currentConfig.CurrentPath -eq $newAgentPath) {
        ...
        Start-AetherAgentService -NSSMPath $nssmPath
    ...
    if ($isAdmin -or $Elevated) {
        ...
        Write-Host ""
        Write-Host "Upgrade completed successfully!" -ForegroundColor Green
        Write-Host "The $ServiceName service has been updated to use the new executable path." -ForegroundColor Green
