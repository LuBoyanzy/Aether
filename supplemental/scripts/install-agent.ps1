...
# Function to find aether-agent (prefers aether name, falls back to legacy package locations)
function Find-AetherAgent {
    ...
    # Common installation paths to check (both new and legacy package names)
    $commonPaths = @(
...
function Install-AetherAgentWithScoop {
    ...
    if (-not (Test-CommandExists $LegacyPackageName)) {
        throw "Failed to install $LegacyPackageName"
    }
    
    return $(Join-Path -Path $(scoop prefix $LegacyPackageName) -ChildPath "$LegacyPackageName.exe")
}

# Function to install agent via WinGet (legacy package id)
function Install-AetherAgentWithWinGet {
    Write-Host "Installing / updating $LegacyPackageName..."
    ...
    & winget install --exact --id $LegacyWingetId --accept-source-agreements --accept-package-agreements | Out-Null
    ...
    if (-not $agentPath) {
        throw "Could not find agent executable path after installation"
    }
...
function Install-WithWinGet {
    ...
    try {
        ...
        # Install agent from legacy WinGet package
        $agentPath = Install-AetherAgentWithWinGet
...
function Install-NSSMService {
    ...
    Write-Host "Installing $ProgramDisplayName service..."
    ...
    # Configure log files
    $logDir = "$env:ProgramData\$ServiceName\logs"
...
try {
    ...
    if (-not $AgentPath) {
        throw "Could not find agent executable. Make sure installation from the legacy package completed successfully."
    }
