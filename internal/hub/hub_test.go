...
func TestMakeLink(t *testing.T) {
	hub, _ := aetherTests.NewTestHub(t.TempDir())
	...
	tests := []struct {
	...
	}{
		{
			...
			appURL:   "http://localhost:19090",
			parts:    []string{},
			expected: "http://localhost:19090",
		},
		{
			name:     "no parts, with trailing slash in AppURL",
			appURL:   "http://localhost:19090/",
			parts:    []string{},
			expected: "http://localhost:19090", // TrimSuffix should handle the trailing slash
		},
		{
			name:     "one part",
...
func TestApiRoutesAuthentication(t *testing.T) {
	...
	user, err := aetherTests.CreateUser(hub, "testuser@example.com", "password123")
	require.NoError(t, err, "Failed to create test user")

	adminUser, err := aetherTests.CreateRecord(hub, "users", map[string]any{
	...
	// superUser, err := aetherTests.CreateRecord(hub, core.CollectionNameSuperusers, map[string]any{
	...
	scenarios := []aetherTests.ApiScenario{
		// Auth Protected Routes - Should require authentication
		{
			...
			URL:             "/api/aether/test-notification",
			ExpectedStatus:  401,
		...
		},
		{
			...
			Method:          http.MethodGet,
			URL:             "/api/aether/universal-token",
			ExpectedStatus:  401,
			ExpectedContent: []string{"requires valid"},
			TestAppFactory:  testAppFactory,
		},
		...
		},
		{
			...
			BeforeTestFunc: func(t testing.TB, app *pbTests.TestApp, e *core.ServeEvent) {
				...
				aetherTests.CreateRecord(app, "alerts", map[string]any{
					"name":   "CPU",
					"system": system.Id,
					"user":   user.Id,
					"value":  80,
		...
		},
		{
			...
			Method:          http.MethodGet,
			URL:             "/api/aether/containers/logs?system=test-system&container=test-container",
			ExpectedStatus:  401,
			ExpectedContent: []string{"requires valid"},
			TestAppFactory:  testAppFactory,
		},
		{
			...
			Method: http.MethodGet,
			URL:    "/api/aether/containers/logs?container=test-container",
			Headers: map[string]string{
				"Authorization": userToken,
			},
			ExpectedStatus:  400,
		...
		},
		{
			...
			Method: http.MethodGet,
			URL:    "/api/aether/containers/logs?system=test-system",
			Headers: map[string]string{
				"Authorization": userToken,
			},
			ExpectedStatus:  400,
		...
		},
		{
			...
			Method: http.MethodGet,
			URL:    "/api/aether/containers/logs?system=invalid-system&container=test-container",
			Headers: map[string]string{
				"Authorization": userToken,
			},
			ExpectedStatus:  404,
...
func TestFirstUserCreation(t *testing.T) {
	...
	t.Run("CreateUserEndpoint not available when USER_EMAIL, USER_PASSWORD are set", func(t *testing.T) {
		os.Setenv("AETHER_HUB_USER_EMAIL", "me@example.com")
		os.Setenv("AETHER_HUB_USER_PASSWORD", "password123")
		defer os.Unsetenv("AETHER_HUB_USER_EMAIL")
		defer os.Unsetenv("AETHER_HUB_USER_PASSWORD")
		...
		scenario := aetherTests.ApiScenario{
...
func TestCreateUserEndpointAvailability(t *testing.T) {
	...
	t.Run("CreateUserEndpoint not available when users exist", func(t *testing.T) {
		...
		// Create a user first
		_, err := aetherTests.CreateUser(hub, "existing@example.com", "password")
		require.NoError(t, err)
...
func TestAutoLoginMiddleware(t *testing.T) {
	...
	scenarios := []aetherTests.ApiScenario{
		...
		},
		{
			...
			TestAppFactory:  testAppFactory,
			BeforeTestFunc: func(t testing.TB, app *pbTests.TestApp, e *core.ServeEvent) {
				aetherTests.CreateUser(app, "user@test.com", "password123")
			},
		},
	}
...
func TestTrustedHeaderMiddleware(t *testing.T) {
	...
	os.Setenv("TRUSTED_AUTH_HEADER", "X-Aether-Trusted")
	...
	scenarios := []aetherTests.ApiScenario{
		...
		},
		{
			...
			URL:    "/api/aether/getkey",
			Headers: map[string]string{
				"X-Aether-Trusted": "user@test.com",
			},
			ExpectedStatus:  401,
			ExpectedContent: []string{"requires valid"},
			TestAppFactory:  testAppFactory,
