...
// GetEnv retrieves an environment variable with an "AETHER_HUB_" prefix, or falls back to the unprefixed key.
func GetEnv(key string) (value string, exists bool) {
	if value, exists = os.LookupEnv("AETHER_HUB_" + key); exists {
		return value, exists
	}
...
func (h *Hub) registerApiRoutes(se *core.ServeEvent) error {
	...
	apiAuth := se.Router.Group("/api/aether")
	apiAuth.Bind(apis.RequireAuth())
	// auth optional routes
	apiNoAuth := se.Router.Group("/api/aether")
	...
	apiAuth.GET("/getkey", func(e *core.RequestEvent) error {
		return e.JSON(http.StatusOK, map[string]string{"key": h.pubKey, "v": aether.Version})
	})
...
// getContainerLogs handles GET /api/aether/containers/logs requests
...
// refreshSmartData handles POST /api/aether/smart/refresh requests
