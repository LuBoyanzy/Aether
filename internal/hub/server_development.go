...
func (rm *responseModifier) modifyHTML(html string) string {
	...
	html = strings.Replace(html, "{{V}}", aether.Version, 1)
	html = strings.Replace(html, "{{HUB_URL}}", rm.hub.appURL, 1)
	return html
}

// startServer sets up the development server for Aether
func (h *Hub) startServer(se *core.ServeEvent) error {
	h.Logger().Info("starting server", "logger", "hub", "appURL", h.appURL)
