...
func TestAgentConnect(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:           "Token too long",
			headers:        map[string]string{"X-Token": strings.Repeat("a", 65), "X-Aether": "0.5.0"},
			expectedStatus: http.StatusBadRequest,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			req := httptest.NewRequest("GET", "/api/aether/agent-connect", nil)
...
func TestHandleAgentConnect(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			req := httptest.NewRequest(tc.method, "/api/aether/agent-connect", nil)
			for key, value := range tc.headers {
				req.Header.Set(key, value)
			}
...
func TestAgentWebSocketIntegration(t *testing.T) {
	...
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path == "/api/aether/agent-connect" {
			acr := &agentConnectRequest{
				hub: hub,
				req: r,
				res: w,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			os.Setenv("AETHER_AGENT_HUB_URL", ts.URL)
			os.Setenv("AETHER_AGENT_TOKEN", tc.agentToken)
			defer func() {
...
func TestMultipleSystemsWithSameUniversalToken(t *testing.T) {
	...
	for i, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Set up environment variables for the agent
			os.Setenv("AETHER_AGENT_HUB_URL", ts.URL)
			os.Setenv("AETHER_AGENT_TOKEN", universalToken)
			defer func() {
				os.Unsetenv("AETHER_AGENT_HUB_URL")
				os.Unsetenv("AETHER_AGENT_TOKEN")
			}()
