...
func TestUserAlertsApi(t *testing.T) {
	...
	user1, _ := aetherTests.CreateUser(hub, "alertstest@example.com", "password")
	user1Token, _ := user1.NewAuthToken()

	user2, _ := aetherTests.CreateUser(hub, "alertstest2@example.com", "password")
	user2Token, _ := user2.NewAuthToken()

	system1, _ := aetherTests.CreateRecord(hub, "systems", map[string]any{
		"name":  "system1",
		"users": []string{user1.Id},
		"host":  "127.0.0.1",
	})

	system2, _ := aetherTests.CreateRecord(hub, "systems", map[string]any{
		"name":  "system2",
	...
	scenarios := []aetherTests.ApiScenario{
		...
		URL:    "/api/aether/user-alerts",
			...
			BeforeTestFunc: func(t testing.TB, app *pbTests.TestApp, e *core.ServeEvent) {
				aetherTests.ClearCollection(t, app, "alerts")
		...
		URL:    "/api/aether/user-alerts",
			...
			BeforeTestFunc: func(t testing.TB, app *pbTests.TestApp, e *core.ServeEvent) {
				...
				for _, systemId := range []string{system1.Id, system2.Id} {
					_, err := aetherTests.CreateRecord(app, "alerts", map[string]any{
						"name":   "Memory",
						"system": systemId,
						"user":   user1.Id,
						"value":  90,
...
func TestStatusAlerts(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		systems, err := aetherTests.CreateSystems(hub, 4, user.Id, "paused")
		assert.NoError(t, err)

		var alerts []*core.Record
		for i, system := range systems {
			alert, err := aetherTests.CreateRecord(hub, "alerts", map[string]any{
				"name":   "Status",
				"system": system.Id,
				"user":   user.Id,
				"min":    i + 1,
...
func TestAlertsHistory(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		systems2, err := aetherTests.CreateSystems(hub, 1, user.Id, "up")
		assert.NoError(t, err)
		system2 := systems2[0]
		...
		alert2, err := aetherTests.CreateRecord(hub, "alerts", map[string]any{
			"name":   "Status",
			"system": system2.Id,
			"user":   user.Id,
			"min":    1,
...
func TestResolveStatusAlerts(t *testing.T) {
	...
	// Create a systemUp
	systemUp, err := aetherTests.CreateRecord(hub, "systems", map[string]any{
		"name":   "test-system",
	...
	systemDown, err := aetherTests.CreateRecord(hub, "systems", map[string]any{
		"name":   "test-system-2",
		"users":  []string{user.Id},
		"host":   "127.0.0.2",
		"status": "up",
	...
	alertUp, err := aetherTests.CreateRecord(hub, "alerts", map[string]any{
		"name":   "Status",
		"system": systemUp.Id,
		"user":   user.Id,
		"min":    1,
	...
	alertDown, err := aetherTests.CreateRecord(hub, "alerts", map[string]any{
		"name":   "Status",
