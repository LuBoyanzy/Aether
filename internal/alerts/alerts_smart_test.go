...
func TestSmartDeviceAlert(t *testing.T) {
	...
	smartDevice, err := aetherTests.CreateRecord(hub, "smart_devices", map[string]any{
		"system": system.Id,
		"name":   "/dev/sda",
...
func TestSmartDeviceAlertMultipleUsers(t *testing.T) {
	hub, user1 := aetherTests.GetHubWithUser(t)
	defer hub.Cleanup()

	// Create a second user
	user2, err := aetherTests.CreateUser(hub, "test2@example.com", "password")
	assert.NoError(t, err)

	// Create user settings for the second user
	_, err = aetherTests.CreateRecord(hub, "user_settings", map[string]any{
		"user":     user2.Id,
