...
func TestAlertSilencedOneTime(t *testing.T) {
	...
	_, err = aetherTests.CreateRecord(hub, "quiet_hours", map[string]any{
		"user":   user.Id,
		"system": system.Id,
		"type":   "one-time",
		"start":  startTime,
...
func TestAlertSilencedGlobal(t *testing.T) {
	...
	systems, err := aetherTests.CreateSystems(hub, 3, user.Id, "up")
	assert.NoError(t, err)
...
func TestAlertSilencedSystemSpecific(t *testing.T) {
	...
	// Create multiple systems
	systems, err := aetherTests.CreateSystems(hub, 2, user.Id, "up")
	assert.NoError(t, err)
	system1 := systems[0]
	system2 := systems[1]
...
func TestAlertSilencedMultiUser(t *testing.T) {
	hub, _ := aetherTests.GetHubWithUser(t)
	defer hub.Cleanup()

	// Create two users
	user1, err := aetherTests.CreateUser(hub, "user1@example.com", "password")
	assert.NoError(t, err)

	user2, err := aetherTests.CreateUser(hub, "user2@example.com", "password")
	assert.NoError(t, err)
...
func TestAlertSilencedWithActualAlert(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		_, err = aetherTests.CreateRecord(hub, "alerts", map[string]any{
			"name":   "Status",
			"system": system.Id,
			"user":   user.Id,
			"min":    1,
		...
		if err != nil || userSettings == nil {
			userSettings, err = aetherTests.CreateRecord(hub, "user_settings", map[string]any{
				"user": user.Id,
