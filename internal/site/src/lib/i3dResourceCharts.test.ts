	...
	items: [
		{
			...
			groups: {
				business: { cpu_percent: 7.5, memory_bytes: 512, memory_rss_bytes: 300, memory_cache_bytes: 100 },
		...
		},
		{
			...
			cpu_cores_used: 0.2,
			memory_bytes: 2048,
			memory_usage_bytes: 2600,
			memory_rss_bytes: 1000,
			memory_cache_bytes: 1200,
			disk_read_bps: 100,
			disk_write_bps: 200,
			network_rx_bps: 300,
			network_tx_bps: 400,
			...
			groups: {
				business: { cpu_percent: 15, memory_bytes: 1536, memory_rss_bytes: 900, memory_cache_bytes: 500 },
				monitor: { cpu_percent: 5, memory_bytes: 512, memory_rss_bytes: 128, memory_cache_bytes: 256 },
			},
			targets: {
				"local.inference.web": {
					name: "推理服务",
...
describe("i3d resource charts", () => {
	test("normalizes timeseries into chart points with timestamps and grouped values", () => {
		...
		expect(points[0].memory_pressure_bytes).toBe(700)
		...
		expect(points[0].business_memory_pressure_bytes).toBe(300)
		expect(points[0].middleware_memory_pressure_bytes).toBe(512)
		expect(points[1].middleware_memory_bytes).toBe(0)
		expect(points[1].monitor_memory_bytes).toBe(512)
		expect(points[1].monitor_memory_pressure_bytes).toBe(128)
		expect(points[0].target_local_inference_web_memory_pressure_bytes).toBe(350)
