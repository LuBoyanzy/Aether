...
> 基于 Beszel 二次开发的服务器与容器监控平台。代码与功能已切换为 Aether 品牌，下载源与镜像来自本仓库。
...
> macOS 可使用 `install-agent-brew.sh`（直接从 GitHub Releases 下载），Windows 使用 PowerShell 版本：`supplemental/scripts/install-agent.ps1`。
